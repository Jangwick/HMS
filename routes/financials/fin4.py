...
SUBSYSTEM_NAME = 'FIN4 - Cash Management'
ACCENT_COLOR = '#7C3AED'
BLUEPRINT_NAME = 'fin4'
        ...
        if user:
            ...
            if user.check_password(password):
                if user.password_expires_at and user.password_expires_at < now_utc:
                    ...
                    return redirect(url_for('fin4.change_password'))
...
@fin4_bp.route('/register', methods=['GET', 'POST'])
def register():
    if request.method == 'POST':
        username = request.form.get('username')
...
@fin4_bp.route('/change-password', methods=['GET', 'POST'])
def change_password():
...
@fin4_bp.route('/settings', methods=['GET', 'POST'])
@login_required
def settings():
    if request.method == 'POST':
        email = request.form.get('email')

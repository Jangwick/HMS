...
from app import db
from models.ct_users import CT3User
...
def login():
    ...
    if request.method == 'POST':
        ...
        user = CT3User.query.filter_by(username=username).first()
...
def change_password():
    ...
    if is_expired:
        user = CT3User.query.get(expired_user_id)
        if not user:
...
def dashboard():
    ...
    return render_template('subsystems/core_transaction/ct3/dashboard.html', now=datetime.utcnow)
